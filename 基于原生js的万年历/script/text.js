		...
		if(sD>-1 && sD<cld.length) {
			...
			if(s.length>0) {
				...
				if(s.length>0) {
					...
					s=cld[sD].solarTerms;
					if(s.length>0) s = s.fontcolor('#ee2c2c');								//节气颜色
				}
			}
...
function initial() {
	...
	document.getElementById("set_year").selectedIndex=tY-1900;
	document.getElementById("set_month").selectedIndex=tM;
	drawCld(tY,tM);
}

//通过按钮更改年月
function pushBtm(key) {
	switch (key){
		case 'YU' :
		...
		case 'MU' :
			...
			break;
		case 'MD' :
			if(document.getElementById("set_month").selectedIndex < 11) {
				document.getElementById("set_month").selectedIndex++;
			}
			else {
...
function showText(v){
	...
	selectDay.innerHTML = '<br />'+cld[d].sYear +'-'+ cld[d].sMonth
	+ '-'+cld[d].sDay +' 星期' + cld[d].week +'<div>'+cld[d].sDay+'</div>'+ '<br />'
	+ '<span>农历' + (cld[d].isLeap?'闰 ':' ')
	+ cld[d].lMonth + ' 月 ' + cld[d].lDay + ' 日<br />'
